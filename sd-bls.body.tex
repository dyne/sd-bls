...
adoption of public status lists \cite{crlcomparison}. The hypothetical
...
\item[ii.] \emph{Non-degeneracy}, meaning that if $g_1\in E_T$
  and $g_2\in E$ are generators of their groups, then
  $e(g_1,g_2)\ne 1_{\mathbb{G}_T}$, where
  $1_{\mathbb{G}_T}$ is the identity element of the group
  $\mathbb{G}_T$;
...
\item Verification phase.\\ For another user that wants to verify the
...
This schema binds the credential presentation to the session data
contained in $t$. The verifier checks $\sigma_t$ with public key $pk_t$
over the tuple $(H : r : \sigma' : t : pk_t)$, and checks $\sigma'$
with public key $A.pk + r + pk_t$. Therefore the values $t$, $pk_t$
and $\sigma_t$ are cryptographically bound to the presented
credential.

An attacker may copy a valid presentation only unchanged and only
within the session policy accepted by the verifier for $t$. To create a
fresh valid presentation for a different session, with a different
$t'$ or $pk_t'$, the attacker would need to produce a new valid
session signature without knowledge of the secret key $sk_t$. Thus any
tampering of the presentation will lead to an invalid credential.
...
$H : r$ and the revocation public key, optionally the claim value $m$ can
be included.
...
\end{split}
\end{equation*}
where $U$ is the mapping of the string $H : r$ in the group $G_1$.\\
...
\begin{itemize}
\item verify the information in $t$ (e.g. timestamp)
...
verification runs approximately at the same speed as other operations.
...
information, but may be dishonest and publish revocation keys
...
the BLS461 curve based on a 461 bit prime, which offers a more
conservative margin for 128 bit security than BLS12-381, in particular
against discrete-log attacks on the pairing target group
...
and Matteo Cristino for their indefatigable work on Zenroom, Luca Di
...
his insights on threat models. We extend our thanks to the anonymous
