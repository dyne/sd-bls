...
protection against this attack can be set in place in two different
and non-exclusive ways:
\begin{enumerate}
\item adopt a key-derivation function like Argon2 \cite{argon2} on
  hash creation, which will add computational costs to issuance and
  verification.
\item have the issuer concatenate a nonce to the credential message
  like $m:nonce$ and then hash it as if it would be the message, then
  make sure the nonce is known to the holder.
\end{enumerate}
